...
module.exports = {
    ...
    async execute(channel, client) {
        ...
        const isBlacklisted = isBlacklistedForCategory(guildData, executor.id, executorRoles, 'webhooks', executor.bot);
        
        if (!isBlacklisted) {
            if (isWhitelistedForCategory(guildData, executor.id, executorRoles, 'webhooks', executor.bot)) return;
        }
        
        const result = isBlacklisted ? { triggered: true, count: 1, limit: 1 } : trackAction(guild.id, executor.id, 'webhookCreate');
        ...
        if (result.triggered) {
            try {
                ...
                if (member) {
                    await member.ban({ reason: isBlacklisted ? '[Anti-Nuke] Blacklisted user created webhook' : '[Anti-Nuke] Spam webhook creation detected' });
                }
