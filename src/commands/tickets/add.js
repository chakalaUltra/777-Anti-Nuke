...
module.exports = {
    ...
    async execute(message, args, client, guildData) {
        ...
        if (!isStaff) {
            return message.reply({ embeds: [errorEmbed('Only support staff can add users to tickets.')] });
        }
        ...
        const member = await message.guild.members.fetch(user.id || user.user?.id).catch(() => null);
        if (!member) {
            return message.reply({ embeds: [errorEmbed('Could not find that user in this server.')] });
        }
        ...
        if (!ticket.addedMembers.includes(member.id)) {
            ticket.addedMembers.push(member.id);
            await ticket.save();
        }

        await message.reply({ embeds: [successEmbed(`${member} has been added to this ticket.`)] });
    },
    ...
    async executeSlash(interaction, client, guildData) {
        ...
        if (!isStaff) {
            return interaction.reply({ embeds: [errorEmbed('Only support staff can add users to tickets.')], ephemeral: true });
        }
        ...
        await interaction.reply({ embeds: [successEmbed(`${member} has been added to this ticket.`)] });
    }
};
